...
class ProgressTracker(object):
    ...
    def delete_topic_progress(self, target_db_table_schema: str, target_db_table_name: str,
                              replay_topic: str) -> None:
        """Delete progress records for a specific topic/table combination.

        Used by redo mode to clear progress so replay starts from the beginning.
        """
        target_table_fq_name: str = \
            f'[{target_db_table_schema.strip()}].[{target_db_table_name.strip()}]'
        cursor = self.db_conn.cursor()
        try:
            cursor.execute(f'''
    DELETE FROM {self.progress_table_fq_name}
    WHERE [source_topic_name] = ?
        AND [target_table_object_id] = OBJECT_ID(?)
        AND [replayer_progress_namespace] = ?
            ''', (replay_topic, target_table_fq_name, self.progress_tracking_namespace))
            ...
            self.db_conn.commit()
            logger.info(f'Deleted {deleted} progress record(s) for topic {replay_topic} -> {target_table_fq_name}')
        finally:
            cursor.close()
