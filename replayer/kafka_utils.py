...
def get_lsn_and_command_id_at_offset(opts: argparse.Namespace, offset: int) -> Tuple[str, int]:
    """Read the message at a specific offset from the all-changes topic and return its (lsn, command_id)."""
    ...
    consumer_conf = build_consumer_config(opts.kafka_bootstrap_servers,
                                          f'replayer-offset-lookup-{int(datetime.now().timestamp())}',
                                          **opts.extra_kafka_consumer_config)
    consumer = Consumer(consumer_conf)

    try:
        tp = TopicPartition(opts.all_changes_topic, 0, offset)
        consumer.assign([tp])

        msg = consumer.poll(timeout=10.0)
        if msg is None:
            raise Exception(f'Could not read message at offset {offset} from topic {opts.all_changes_topic}')

        err = msg.error()
        if err:
            raise Exception(f'Error reading from topic {opts.all_changes_topic} at offset {offset}: {err}')

        raw_val = msg.value()
        if raw_val is None:
            raise Exception(f'Message at offset {offset} in topic {opts.all_changes_topic} has null value')
        ...
        logger.info(f'Read (lsn={msg_lsn}, command_id={msg_command_id}) at offset {offset} '
                    f'from topic {opts.all_changes_topic}')
        return msg_lsn, msg_command_id
