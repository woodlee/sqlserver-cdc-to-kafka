...
def _reconnect(opts: argparse.Namespace, metadata: TableMetadata, config: ReplayConfig) -> ctds.Connection:
    """Create a fresh DB connection and recreate temp tables (lost with the old session)."""
    logger.warning('Reconnecting to target database...')
    conn = ctds.connect(opts.target_db_server, user=opts.target_db_user, password=opts.target_db_password,
                        database=opts.target_db_database, timeout=30, login_timeout=30, autocommit=True)
    metadata.create_temp_tables(conn)
    logger.info('Reconnected to target database for topic %s', config.replay_topic)
    return conn
...
def replay_worker(config: ReplayConfig, opts: argparse.Namespace, stop_event: EventClass, queue: Queue,
                  ...
                  cutoff: Optional[Tuple[str, int]] = None, tables_complete_counter: Optional[Synchronized[int]] = None,
    ...
    If cutoff is provided as (lsn, command_id), the worker stops when it sees a message
    whose (lsn, command_id) exceeds the cutoff (i.e., the cutoff is inclusive).
    ...
    db_conn: Optional[ctds.Connection] = None
    try:
        ...
        while True:
            ...
            # Check cutoff (lsn, command_id) in backfill/redo mode
            reached_cutoff = False
            if cutoff is not None and msg_val is not None:
                msg_lsn: str = msg_val['__log_lsn']
                msg_command_id: int = msg_val.get('__command_id', 0)
                if msg_lsn and (msg_lsn, msg_command_id) > cutoff:
                    logger.info(f"Worker {config.replay_topic}: reached cutoff at offset {msg_offset} "
                               f"(msg ({msg_lsn}, {msg_command_id}) > cutoff {cutoff}). Will flush and stop.")
                    reached_cutoff = True
                    ...
                    for attempt in range(MAX_DB_RETRIES):
                        try:
                            ...
                            if attempt >= MAX_DB_RETRIES - 1:
                                raise
                            logger.warning('Delete statement execution failed (attempt %s/%s): %s. Retrying...',
                                           attempt + 1, MAX_DB_RETRIES, e)
                            time.sleep(DB_RETRY_DELAY_SECS)
                            db_conn = _reconnect(opts, metadata, config)
                            db_conn.bulk_insert(metadata.delete_temp_table_name, deletes, tablock=True)
                    sql_time_acc += time.perf_counter() - start_time
                    ...
                    if merges:
                        ...
                        for attempt in range(MAX_DB_RETRIES):
                            try:
                                ...
                                if attempt >= MAX_DB_RETRIES - 1:
                                    raise
                                logger.warning('Merge statement execution failed (attempt %s/%s): %s. Retrying...',
                                               attempt + 1, MAX_DB_RETRIES, e)
                                time.sleep(DB_RETRY_DELAY_SECS)
                                db_conn = _reconnect(opts, metadata, config)
                                db_conn.bulk_insert(metadata.merge_temp_table_name, merges, tablock=True)
